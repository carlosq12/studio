...
import type { Task, IngresoFuncionario } from '@/lib/types';
...
export function EditTaskDialog({ task, open, onOpenChange }: EditTaskDialogProps) {
  ...
  useEffect(() => {
    if (task) {
      form.reset({
        ...task,
        fecha_unica: task.fecha_inicio ? parseISO(task.fecha_inicio) : new Date(),
        fecha_inicio_semanal: task.fecha_inicio ? parseISO(task.fecha_inicio) : new Date(),
        fecha_fin_semanal: task.fecha_fin ? parseISO(task.fecha_fin) : undefined,
        descripcion: task.descripcion || '',
        lugar: task.lugar || '',
        correo: task.correo || '',
        prioridad: task.prioridad || 'Media',
        persona: Array.isArray(task.persona) ? task.persona : (task.persona ? [task.persona] : []),
        tipo_tarea: task.tipo_tarea || 'Día Único',
  ...
  async function onSubmit(data: TaskFormValues) {
    ...
    try {
      ...
      const result = await updateTask(taskData as any);
      ...
      toast({
        ...
        description:
          error.message || 'No se pudo actualizar la tarea. Inténtalo de nuevo.',
      });
    } finally {
