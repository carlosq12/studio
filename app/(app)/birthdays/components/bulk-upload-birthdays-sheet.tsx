...
import { addMultipleBirthdays } from '../actions';
import {
  Table,
  TableBody,
  TableCell,
...
const columnOrder: (keyof Omit<BirthdayData, 'fecha nacimiento'> | 'fecha nacimiento (texto)')[] = [
    'nombre funcionario',
    'fecha nacimiento (texto)',
    'correo'
];

export function BulkUploadBirthdaysSheet() {
  const [open, setOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [file, setFile] = useState<File | null>(null);
  const [fileName, setFileName] = useState('');
  const [parsedData, setParsedData] = useState<BirthdayData[]>([]);
  ...
  const parseExcel = (fileToParse: File) => {
    ...
    reader.onload = (e) => {
      ...
      if (typeof data === 'string' || data instanceof ArrayBuffer) {
        ...
        const birthdays: BirthdayData[] = rows.map(row => {
            ...
            if (typeof dateValue === 'number') {
                birthDate = excelSerialDateToJSDate(dateValue);
            ...
            return {
                ...
                'fecha nacimiento': birthDate || new Date(), // Fallback to a default date
                correo: email
            };
        }).filter(b => b['nombre funcionario'] && b['fecha nacimiento'] && !isNaN(b['fecha nacimiento'].getTime()));

        setParsedData(birthdays);
      }
    };
  ...
  async function onSubmit() {
    if (parsedData.length === 0) {
      toast({
        ...
        description: 'Por favor, selecciona un archivo de Excel válido con cumpleaños.',
      });
      return;
    }
    ...
    try {
      const result = await addMultipleBirthdays(parsedData);
      ...
      toast({
        ...
        description: `${result.count} cumpleaños han sido añadidos a la lista.`,
      });
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      ...
      <SheetContent className="sm:max-w-xl">
        <SheetHeader>
          <SheetTitle>Carga Masiva de Cumpleaños</SheetTitle>
          <SheetDescription>
            Sube un archivo de Excel (.xlsx, .xls, .csv) para añadir múltiples
            cumpleaños. Las columnas deben ser: Nombre Funcionario, Fecha Nacimiento, Correo.
          </SheetDescription>
        </SheetHeader>
                ...
                <Table>
                  ...
                  <TableBody>
                    {parsedData.slice(0, 5).map((row, i) => (
                      <TableRow key={i}>
                        <TableCell>{row['nombre funcionario']}</TableCell>
                        <TableCell>{row['fecha nacimiento'].toLocaleDateString()}</TableCell>
