...
import { Loader2, CalendarIcon, Monitor, Printer as PrinterIcon, Server, Tv, Laptop, Router, HardDrive } from 'lucide-react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { z } from 'zod';
import { useToast } from '@/hooks/use-toast';
import { useState, useEffect, useMemo } from 'react';
import { updateInventarioEquipo } from '../actions';
import type { InventarioEquipo, IngresoFuncionario } from '@/lib/types';
...
export function EditEquipoDialog({ equipo, open, onOpenChange }: EditEquipoDialogProps) {
  ...
  useEffect(() => {
    if (equipo) {
      form.reset({
        ...
        tipo_arriendo: equipo.tipo_arriendo || '',
        'fecha de ingreso': parseDate(equipo['fecha de ingreso']),
        'correo relacionado': equipo['correo relacionado'] || '',
      });
    }
  }, [equipo, form]);
  ...
  async function onSubmit(data: EquipoFormValues) {
    ...
    try {
      const result = await updateInventarioEquipo({ ...data, id: equipo.id, 'fecha de ingreso': data['fecha de ingreso'] as any });
      if (result?.error) {
        throw new Error(result.error);
      }
      toast({
        title: '¡Equipo actualizado!',
        description: `El equipo "${data['nombre equipo']}" ha sido actualizado.`,
      });
      onOpenChange(false);
    } catch (error: any) {
      toast({
        ...
        description: error.message || 'No se pudo actualizar the equipo.',
      });
    } finally {
  ...
  const handleIpChange = (e: React.ChangeEvent<HTMLInputElement>, fieldName: 'ip equipo' | 'dns1' | 'dns2' | 'puerta de enlace ipv4' | 'mascara ipv4') => {
    ...
    let newVal = parts.map((part) => part.substring(0, 3)).join('.');
  ...
  return (
    ...
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>Editar Equipo del Inventario</DialogTitle>
          <DialogDescription>
            Modifica los detalles del equipo. Haz clic en guardar cuando termines.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
