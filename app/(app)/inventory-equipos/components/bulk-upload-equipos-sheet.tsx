...
import { addMultipleInventarioEquipos } from '../actions'; 
import {
  Table,
  TableBody,
  TableCell,
...
const columnMap: Record<string, keyof InventarioEquipoData> = {
  ...
  'DNS1': 'dns1',
  'DNS2': 'dns2',
  'PUERTA DE ENLACE IPV4': 'puerta de enlace ipv4',
  'MASCARA IPV4': 'mascara ipv4',
  'LICENCIA OFFICE': 'licencia office',
  'PERSONAL A CARGO': 'personal a cargo',
  'USUARIO DEL ENCARGADO': 'usuario del encargado',
  'UBICACION': 'ubicacion',
...
export function BulkUploadEquiposSheet() {
  const [open, setOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [fileName, setFileName] = useState('');
  const [parsedData, setParsedData] = useState<InventarioEquipoData[]>([]);
    ...
    reader.onload = (e) => {
      try {
        ...
        if (typeof data === 'string' || data instanceof ArrayBuffer) {
          ...
          const headerRow: string[] = json[0].map(h => normalizeHeader(h));
          const dataRows = json.slice(1);
          
          const equipos: InventarioEquipoData[] = dataRows.map((row): InventarioEquipoData | null => {
              const rowData: Partial<InventarioEquipoData> = {};
              headerRow.forEach((header, index) => {
                  const mappedKey = columnMap[header];
                  if(mappedKey) {
                    if(mappedKey === 'fecha de ingreso') {
                        (rowData as any)[mappedKey] = parseDate(row[index]);
                    } else {
                        (rowData as any)[mappedKey] = row[index] ? String(row[index]) : '';
                    }
                  }
              });
              ...
              return rowData as InventarioEquipoData;
          }).filter((f): f is InventarioEquipoData => f !== null);

          setParsedData(equipos);

          if (equipos.length === 0 && dataRows.length > 0) {
            toast({
                variant: 'destructive',
                title: 'No se encontraron registros válidos',
                description: 'Asegúrate de que al menos una columna tenga datos en cada fila.',
                duration: 9000,
            });
          }
        }
  ...
  async function onSubmit() {
    if (parsedData.length === 0) {
      toast({
        ...
        description: 'Por favor, selecciona un archivo de Excel válido con equipos.',
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: `${result.count} equipos han sido añadidos al inventario.`,
      });
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      ...
      <SheetContent className="sm:max-w-xl">
        <SheetHeader>
          <SheetTitle>Carga Masiva de Equipos</SheetTitle>
           <SheetDescription>
            Sube un archivo Excel. El sistema leerá los encabezados de la primera fila. Los nombres de las columnas deben coincidir con los campos del formulario.
            <br/> <strong className="font-bold text-foreground mt-2 block">Nombres de columnas esperados:</strong> {Object.keys(columnMap).join(', ')}.
          </SheetDescription>
        </SheetHeader>
                ...
                <Table>
                  ...
                  <TableBody>
                    {parsedData.slice(0, 5).map((row, i) => (
                      <TableRow key={i}>
                        <TableCell>{row['nombre equipo']}</TableCell>
          ...
          >
            {isSubmitting ? (
              <>
                ...
                Cargar {parsedData.length} Equipos
                </>
            )}
          </Button>
        </SheetFooter>
