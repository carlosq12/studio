...
import { addMultipleInventoryItems } from '../actions';
import {
  Table,
  TableBody,
  TableCell,
...
const columnMap: Record<string, keyof InventoryItemData> = {
  'NOMBRE': 'nombre',
...
export function BulkUploadInventorySheet() {
  const [open, setOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [fileName, setFileName] = useState('');
  const [parsedData, setParsedData] = useState<InventoryItemData[]>([]);
    ...
    reader.onload = (e) => {
      try {
        ...
        if (typeof data === 'string' || data instanceof ArrayBuffer) {
          ...
          const items: InventoryItemData[] = dataRows.map((row, rowIndex): InventoryItemData | null => {
              const rowData: Partial<InventoryItemData> = {};
              headerRow.forEach((header, index) => {
                  const mappedKey = columnMap[header];
                  if(mappedKey) {
                    if(mappedKey === 'fecha de ingreso') {
                        (rowData as any)[mappedKey] = parseDate(row[index]);
                    } else if (mappedKey === 'cantidad') {
                        (rowData as any)[mappedKey] = typeof row[index] === 'number' ? row[index] : parseInt(String(row[index] || '0'));
                    }
                    else {
              ...
              if (!rowData.nombre || String(rowData.nombre).trim() === '') {
                 return null; 
              }
              
              return rowData as InventoryItemData;
          }).filter((f): f is InventoryItemData => f !== null);
          ...
          if (items.length === 0 && dataRows.length > 0) {
            toast({
                ...
                description: 'Revisa que la columna "nombre" no esté vacía en ninguna fila.',
                duration: 9000,
            });
          }
        }
  ...
  async function onSubmit() {
    if (parsedData.length === 0) {
      toast({
        ...
        description: 'Por favor, selecciona un archivo de Excel válido con items de inventario.',
      });
      return;
    }
    ...
    try {
      const result = await addMultipleInventoryItems(parsedData as any);
      ...
      toast({
        ...
        description: `${result.count} items han sido añadidos al inventario.`,
      });
          ...
          >
            {isSubmitting ? (
              <>
                ...
                Cargar {parsedData.length} Items
                </>
            )}
          </Button>
        </SheetFooter>
